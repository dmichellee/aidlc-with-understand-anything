...
<p>⚠️ <strong>주의</strong>: 이 프롬프트를 사용하기 전에 <code>docs/</code> 폴더 내 문서들의 <strong>정확성과 최신성을 반드시 리뷰</strong>하세요. AI-DLC 워크플로우는 해당 문서를 근거로 분석·설계·코드 생성을 수행하므로, 오래되거나 부정확한 문서가 포함되면 산출물에 오류가 전파됩니다.</p>
</div>
