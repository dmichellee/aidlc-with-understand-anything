...
> **Kiro에게 시킬 프롬프트**
> ```
> 수납업무 개선을 위해 카드 자동이체 기능을 개선하고 싶어.
> '카드-자동이체-개선.md' 파일의 요구사항을 보고 개발을 진행해줘.
<!-- > docs/ 폴더에 기존 개발 및 운영 관련 문서를 넣어두었으니 이걸 reverse engineering 문서로 쓰고 해당 단계는 넘어가자. -->
> docs/ 폴더에 최신 버전의 기존 개발 및 운영 관련 문서를 넣어두었어. reverse engineering을 수행할 땐 여기 있는 문서를 기반으로 하고, 부족한 정보만 추가로 탐색해.
> AI-DLC 워크플로우로 진행해줘.
> ```

> ⚠️ **주의**: 이 프롬프트를 사용하기 전에 `docs/` 폴더 내 문서들의 **정확성과 최신성을 반드시 리뷰**하세요. AI-DLC 워크플로우는 해당 문서를 근거로 분석·설계·코드 생성을 수행하므로, 오래되거나 부정확한 문서가 포함되면 산출물에 오류가 전파됩니다.
...
> ```
> 리버스 엔지니어링 결과 확인했어. 정확해. 다음 단계 진행해줘.
> ```
...
> **Kiro가 각 단계 별로 필요한 질문이나 작업 요청을 적절히 수행합니다.**
>
> **만약 특정 항목에 대해 더 자세히 확인이 필요하거나, 추가적으로 knowledge graph를 조회하고 싶을 때는**
>
> **Kiro에게 'kg-explain', 'kg-search' 등을 활용하도록 명시적으로 조회를 요청합니다**
