...
> ⚠️ `/understand`는 **생성 전용**입니다. 코드 검색·이해는 3장에서 만들 `kg-search`/`kg-explain`으로 합니다.

## ⌨️ 실습 1 — 설치 및 사전 요구사항 확인
...
> ⚠️ 설치 후 **Kiro CLI(또는 IDE)를 재시작**해야 스킬·에이전트가 로드됩니다.
...
> ```
> 단, 이 워크샵은 그래프를 **생성**한 뒤 3장에서 만드는 `aidlc-main` 에이전트로 **활용**하는 흐름입니다.
...
브라우저에서 노드(파일/함수/클래스)와 엣지(관계), 레이어, 투어를 인터랙티브하게 탐색할 수 있습니다. 레거시의 전체 구조를 처음 조망하기에 가장 좋은 출발점입니다.
...
레거시는 보통 여러 모듈로 쪼개져 있습니다(예: 백엔드 API, 비즈니스 로직, 프론트 등). 이럴 때는 **모듈별로 그래프를 생성**합니다.
...
- [ ] `node -v`, `pnpm -v`가 요구 버전을 만족한다
- [ ] `~/.kiro/skills/`에 understand 계열 스킬이 있다
...
| `pnpm: command not found` | `npm install -g pnpm` 후 재시도 |
| Node 버전이 낮음 | Node 22+ 설치 (nvm 사용 권장) |
| 파일이 100개 초과 경고 | 정상. 서브디렉토리로 범위를 좁히거나 시간이 걸려도 진행 |
