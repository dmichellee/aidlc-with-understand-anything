...
레거시 시스템을 넘겨받았는데 문서도, 원 개발자도 없다면? 코드는 남아 있지만 **"왜 이렇게 짜였는지"에 대한 지식(오너십)은 유실**된 상태입니다. 이런 시스템을 **Brownfield(브라운필드)** 라고 부릅니다.
...
대규모 레거시에 AI 에이전트를 그냥 붙이면 두 가지 문제가 생깁니다.
...
> 💡 이 워크샵의 예제는 다중 모듈로 구성된 레거시 리포(Spring 백엔드 + JSP 프론트 등)를 가정합니다. 여러분의 실제 프로젝트로 진행해도 무방합니다.
